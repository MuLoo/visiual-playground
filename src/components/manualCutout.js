...
// @ts-nocheck
import $ from "jquery";
export default function penCutout() {
  this.defaults = {
    drawPanel: "drawPanel",
    canvasId: "canvas",
    imgId: "imgCut",
  ...
  this.drawAllLine = function () {
    for (let i = 0; i < this.can.pointList.length - 1; i++) {
      // 画线
      const p = this.can.pointList;
    ...
    if (this.can.IsClose) {
      ...
      for (let i = 0; i < p.length - 1; i++) {
        ...
        if (parseFloat(p[i + 1].pointx) - parseFloat(p[i].pointx) !== 0) {
          ...
          const b = p[i].pointy - k * p[i].pointx;
          const userK = parseFloat(k * newx + b);
          if (
            ...
            const aa = Math.abs(p[i + 1].pointx - p[i].pointx - 3);
            const ab = Math.abs(p[i + 1].pointx - newx);
            const ac = Math.abs(newx - p[i].pointx);
